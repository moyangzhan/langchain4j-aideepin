...
public class AdiCommonChatRetrievalAugmentor implements RetrievalAugmentor {
    ...
    @Builder
    public AdiCommonChatRetrievalAugmentor(QueryTransformer queryTransformer,
    ...
    public AugmentationResult augment(AugmentationRequest augmentationRequest) {
        ...
        for (Map.Entry<Query, Collection<List<Content>>> entry : queryToContents.entrySet()) {
            log.info("增强后的用户问题:{}", entry.getKey().text());
            newQuestion.append(entry.getKey().text()).append("\n\n");
            queryToContents.put(entry.getKey(), Collections.emptyList());
        }
        chatMessage = new UserMessage(chatMessage.text() + "\n\n结合下面的问题一起回答,注意,答案中不能体现出你回答了以下问题:\n\n" + newQuestion + "\n\n");
    ...
    public static AdiCommonChatRetrievalAugmentorBuilder builder() {
        return new AdiCommonChatRetrievalAugmentorBuilder();
    }

    public static class AdiCommonChatRetrievalAugmentorBuilder {

        public AdiCommonChatRetrievalAugmentorBuilder contentRetriever(ContentRetriever contentRetriever) {
