 ...
 */
public class AdiDefaultRetrievalAugmentor implements RetrievalAugmentor {

    private static final Logger log = LoggerFactory.getLogger(AdiDefaultRetrievalAugmentor.class);
...
//    private final QueryRouter queryRouter;
    ...
    @Builder
    public AdiDefaultRetrievalAugmentor(QueryTransformer queryTransformer,
...
//        Map<Query, Collection<List<Content>>> queryToContents = join(queryToFutureContents);
        ...
        queries.forEach(query -> {
            log.info("增强后的用户问题:{}", query.text());
            queryToContents.put(query, Collections.emptyList());
        });
    ...
    public static AdiDefaultRetrievalAugmentorBuilder builder() {
        return new AdiDefaultRetrievalAugmentorBuilder();
    }

    public static class AdiDefaultRetrievalAugmentorBuilder {

        public AdiDefaultRetrievalAugmentorBuilder contentRetriever(ContentRetriever contentRetriever) {
