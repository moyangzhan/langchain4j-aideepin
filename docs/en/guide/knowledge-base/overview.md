...
| 3. Index | Vectorization (required) and graph extraction (optional) | [Import Documents](document.md) |
...
> Q&A mode only imports XLSX / XLS / CSV files; parent-child mode has its own child max-token setting.

## Two Index Types

| Index | Purpose | Note |
|---|---|---|
| **Vector index** | Embeds segments for semantic similarity search | The foundation for retrieval |
| **Graph index** | Extracts entities and relations into a knowledge graph | Optional; extraction uses an LLM and **consumes tokens** |

Both can coexist; answers can be traced via citations and the citation graph (see [Knowledge Graph](graph.md)).

## Strict vs Lenient Mode

A knowledge-base-level answering strategy:

- **Strict**: with no relevant hits, returns "[No answer]" instead of making things up — for policies and compliance scenarios;
- **Lenient**: with no hits, the question goes to the LLM for a free-form answer.
...
- **Knowledge base Q&A**: the **Knowledge Base** menu item, ask directly;
