...
- 使用 IDE 默认格式化即可
- JDK 17 语法特性
- **兼容性/过渡代码的注释规范**：存量迁移、历史数据兜底等过渡性组件（如 `SegmentNeo4jBackfillRunner`）的类级 Javadoc 必须写清五点，缺一不可：
  1. **定位**：标注"兼容性代码"，说明它对应的正式迁移（如 SQL 迁移编号）与生效条件（如配置开关）；
  2. **目的**：因哪次重构而存在、解决什么问题；
  3. **行为**：做什么，输入输出是什么；
  4. **安全约束**：不可逆操作的顺序保证（如"先确认落库、再清空源数据"）、幂等与崩溃恢复语义；
  5. **删除时机**：给出可验证的判定标准（日志特征/查询语句），满足即可整体删除。
