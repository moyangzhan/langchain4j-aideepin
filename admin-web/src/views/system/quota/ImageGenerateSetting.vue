  ...
  <n-grid cols="2 s:2 m:2 l:3 xl:3 2xl:3" responsive="screen">
    <n-grid-item>
      <n-form :label-width="80" :model="formValue" :rules="rules" ref="formRef">
        <n-form-item :label="t('system.dailyImageCount')" path="daily">
          <n-input-number v-model:value="formValue.daily" :min="0" />
        </n-form-item>

        <n-form-item :label="t('system.monthlyImageCount')" path="monthly">
...
<script lang="ts" setup>
  ...
  watch(
    () => props.quota.daily,
    () => {
      console.log('imageGenerated', props.quota)
  ...
  function formSubmit() {
    formRef.value.validate(async (errors) => {
      await api.edit({ name: 'quota_by_image_daily', value: formValue.daily })
      await api.edit({ name: 'quota_by_image_monthly', value: formValue.monthly })
      if (!errors) {
        reloadConfig()
